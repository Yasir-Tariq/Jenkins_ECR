     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
                ...
                new_task_definition = sh(script: "${env.task_definition} | jq --arg IMAGE ${env.ecr_image} '.taskDefinition | .containerDefinitions[0].image = $IMAGE | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
