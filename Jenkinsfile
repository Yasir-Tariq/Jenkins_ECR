            ...
            steps {
                ...
                withAWS(region:'us-east-2') {
                    sh '''ECR_IMAGE="020046395185.dkr.ecr.us-east-2.amazonaws.com/tweet:\${GIT_COMMIT}"
                        TASK_DEFINITION=$(aws ecs describe-task-definition --task-definition "app-db-cfn" --region "us-east-2")
                        ...
                        aws ecs update-service --cluster "mycluster" --service "sample-service" --task-definition "app-db-cfn":\${NEW_REVISION}
                '''
                }
                // }
