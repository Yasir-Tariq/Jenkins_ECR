     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
               ...
               new_task_definition = sh(script: "echo ${env.task_definition} | jq --arg imagee ${env.ecr_image} '.taskDefinition | .containerDefinitions[0].image = $imagee | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
