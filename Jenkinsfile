     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
               ...
               new_task_definition = sh(script: "echo ${env.task_definition} | jq --arg image ${env.ecr_image} '.taskDefinition | .containerDefinitions.image = $image | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
