     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
                ...
                task_definition = sh(script: "aws ecs describe-task-definition --task-definition ${params.family} --region 'us-east-2' | jq --arg IMAGE ${env.ecr_image} '.taskDefinition | .containerDefinitions[0].image = $ARGS.IMAGE | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
            ...
            steps {
                script {
                    withAWS(region:'us-east-2') {
                        ...
                        // new_task_definition = sh "echo ${task_definition} | jq -r --arg IMAGE ${ecr_image} '.taskDefinition | .containerDefinitions[0].image = $IMAGE | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'"
