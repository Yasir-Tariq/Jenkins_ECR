     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
                ...
                task_definition = sh(script: "aws ecs describe-task-definition --task-definition ${params.family} --region 'us-east-2' | jq   '.taskDefinition | .containerDefinitions[0].image = ${env.ecr_image} | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
