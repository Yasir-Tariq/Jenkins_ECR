     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
               ...
               new_task_definition = sh(script: "echo ${env.task_definition} | jq --arg IMAGE 020046395185.dkr.ecr.us-east-2.amazonaws.com/tweet:e779c3c08851f6b8b262af6e0b55744bfeea2200 '.taskDefinition | .containerDefinitions[0].image = $IMAGE | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
