     ...
     stages {
        ...
        stage ('ecs update'){
            environment {
                ...
                task_definition = sh(script: "aws ecs describe-task-definition --task-definition ${params.family} --region 'us-east-2'", returnStdout: true)
                new_task_definition = sh(script: "cat ${env.task_definition} | jq --arg IMAGE ${env.ecr_image} '.taskDefinition | .containerDefinitions[0].image = $IMAGE | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)'")
